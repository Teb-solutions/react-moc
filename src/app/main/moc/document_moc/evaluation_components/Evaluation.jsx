...
const Evaluation = ({
  ...
  const [handelUrlChange, setHandelUrlChange] = useState({
    urlRemarks: "", // Initialize with an empty string
  });

  useEffect(() => {
    setHandelUrlChange({
      urlRemarks: contentDetails?.remarks || "", // Update the state whenever contentDetails changes
    });
  }, [contentDetails]);
  ...
  const handleUrlChange = (event) => {
    ...
    // Update the errors state with the URL validation error if it exists
  ...
  return (
    <Grid container spacing={2}>
      <Grid item xs={12} md={canEdits ? 8 : 12}>
        <Paper
                    ...
                    <FormControl fullWidth>
                      <Autocomplete
                        ...
                        value={
                          docStaff.find(
                            (staff) =>
                              staff.value === form.data.consultedStaffId
                          ) || null
                        }
              ...
              <>
                ...
                >
                  <FormControl
                    fullWidth
                    sx={{ mt: 1, mb: 1 }}
                    error={!!errorsUrl.urlRemarks} // Display error styling if there's an error
                  >
                    <span>
                    ...
                    <OutlinedInput
                      id="documentUrl"
                      value={handelUrlChange.urlRemarks} // Bind the input to the state variable
                      onChange={handleUrlChange} // Handle input change and validate URL format
                      placeholder="Enter the new URL"
                    />
                    {!!errorsUrl.urlRemarks && (
                      <FormHelperText>{errorsUrl.urlRemarks}</FormHelperText> // Show validation error message
                    )}
                  </FormControl>
                </Box>
                    ...
                    <div className="font-semibold">
                      <a
                        href={
                          ...
                          contentDetails.remarks.startsWith("http")
                            ? contentDetails.remarks
                            : `http://${contentDetails?.remarks}`
                        }
                        target="_blank"
      ...
      {canEdits && (
        <Grid item xs={12} md={4}>
          <Paper
            className="w-full  mx-auto sm:my-8 lg:mt-0  rounded-16 shadow overflow-hidden"
            style={{ width: "100%" }}
          >
            <div
              _ngcontent-fyk-c288=""
            ...
            <div className="prose p-30 pt-24 pb-24">
              <ul className="text-sm">
                <li>
                  ...
                  document by going to MOC details from Tasks page which
                  contains the document share point link and they need to add
                  their comments in the document uploaded in share point.
                </li>
                <li>
                  Once they have reviewed the document, each stakeholder has to
                  go to Tasks page and submit the task as an acknowledgement.
                </li>
                <li>
                ...
                <li>
                  ...
                  incorporating all modifications / suggestions into same
                  location in share point.
                </li>
                <li>
                  Document author has to update the new consolidated document
                  url here.
                </li>
