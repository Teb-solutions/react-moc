...
const EvaluationApproval = ({
  ...
  const loadRiskAnalysisChart = (tasks) => {
    ...
    tasks.forEach((task) => {
      ...
      if (task.riskAnalysisList && task.riskAnalysisList.length > 0) {
        ...
        const sourceTaskId = task.sourceTaskId || highestSourceTaskId;

        // Push to taskLabels to ensure we always have a value
        taskLabels.push(sourceTaskId);
          ...
          ra.riskAnalysisSubTasks.forEach((subTask) => {
            subTask.riskAnalysisHazardTypes.forEach((hazardType) => {
              hazardType.riskAnalysisHazardSituation.forEach((situation) => {
                const residualRisk = situation.residualRisk || 0;
                // Take the highest residual risk value
                taskResidualRisk = Math.max(taskResidualRisk, residualRisk);
              });
            });
          });
        });

        // Add the highest residual risk for this task to the array
        taskResidualRisks.push(taskResidualRisk);
      }
    });

    // Ensure both arrays have values before updating chart options
    if (taskLabels.length > 0 && taskResidualRisks.length > 0) {
      setShowRiskAnalysisChart(true);
      setRiskAnalysisChartOptions((prevOptions) => ({
        ...prevOptions,
        ...
        annotations: {
          yaxis: [
            { y: 400, y2: 800, borderColor: "#fff", fillColor: "#fff", opacity: 0.2 },
            { y: 200, y2: 400, borderColor: "#fff", fillColor: "#fff", opacity: 0.2 },
          ],
        },
        ...
        xaxis: {
          title: { text: "Task ID" },
          categories: [highestSourceTaskId],
        },
        yaxis: {
      ...
      console.error("No valid task labels or residual risks found.");
    }
  ...
  const validateFormRes = () => {
    let tempErrorsDoc = {};
    if (!selectedRespFile.name)
      tempErrorsDoc.name = "File is required";

    if (!selectedRespFile.descritpion)
