...
const ImplementationApprovalSite = ({
  ...
  const [impTaskComments, setImpComments] = useState([]);
  const [impTaskDocArray, setImpTaskDocArray] = useState([{}]);
  ...
  const handelComments = (taskid) => {
    ...
    apiAuth
      ...
      .then((resp) => {
        ...
        //how to get the last comment id
        const lastComment = comments[comments.length - 1];
        const id = lastComment.id;
        
        apiAuth
            .get(`/DocumentManager/DocList/${id}/Task?changeRequestToken=${assetEvaluationId}`)
            .then((documentResp) => {
              
              const count = documentResp.data.data; // Assuming this is the count value
              const newObj = {
                taskid : taskid,
                documents: documentResp.data
              }
              console.log(newObj);
              //this new object will be added to the impTaskDocArray array
              setImpTaskDocArray((prevArray) => [...prevArray, newObj]);

             
              
            });
            console.log(impTaskDocArray, "impTaskDocArray");
      })
      .catch((error) => {
                      ...
                      <td className="task-table-cell mat-cell">
                        ...
                        <div className="task-details p-0 mb-10 pb-10 mt-10">
                          ...
                          >
                            ...
                            <Button style={{width:'100px;'}} variant="contianed" onClick={()=>{
                                  handelComments(imptsk.id)
                                }}>
                                  
                                  Documents</Button>
                                  {
                                    impTaskDocArray.length>0 && (
                                      <div>
                                        <span className="text-sm text-secondary leading-none pt-5">
                                         
                                          {
                                           
                                            impTaskDocArray.filter((task) => task.taskid === imptsk.id).map((task) => {
                                              return (
                                                <div>
                                                 
                                                    {task.documents.data.map((doc) => {
                                                      return (
                                                        <div>
                                
                                                          <button className="italic cursor my-5" onClick={() => HandleDownload(doc.token, doc.name)}>{doc.name}</button>
                                                        </div>
                                                      )
                                                    })}
