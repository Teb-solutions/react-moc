...
const DocumentManager = ({
  ...
  const drawerStyle = ({ open }: { open: boolean }) => ({
    width: 350,
    bgcolor: "background.paper",
  ...
  const formatDate = (dateString: string) => {
    ...
    try {
      const newdate = dayjs(dateString).format("MMMM dd, yyyy");
      return newdate;
    } catch (error) {
  ...
  const handleDelete = (e: any, docId: string, docToken: string) => {
    e.preventDefault();
    console.log("Delete");
  }
  const handelDetailDoc = (doc:any) => {
      ...
      <Fade in={open}>
        <Box sx={style}>
          ...
          >
            ...
            >
              {/* <FuseSvgIcon size={20}>heroicons-outline:x</FuseSvgIcon> */}X
            </Button>
          </div>
            ...
            <Box sx={drawerStyle(fileDetails)}>
              ...
              <div
                ...
                style={{
                  marginTop: "15px",
                  justifyContent:
                    step === 1 ? "center" : canExecute ? "end" : "center",
                  backgroundColor: " rgba(248,250,252)",
                  padding: "10px",
                }}
              >
                <Button
                  ...
                  type="submit"
                  onClick={(e) =>
                    HandleDownload(documenDowToken, selectedDocument?.name)
                  }
                >
                  Download
                </Button>
